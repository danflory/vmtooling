#!/usr/bin/env python3
"""TP-206 runner (pre-fix) for SPR-1137.

Executes the six TP-206 steps against the PRE-FIX tree and records the result as a
pre_fix test execution (the TDD evidence G-TDD-1 requires).
...
def main() -> int:
    ...
    fixture = OW5 / "firecontrol/sql/V999__tp206_fixture.sql"
    ...
    src = gate.read_text()
    venv_relative = "Path(sys.executable).parent" in src
    record(2, "V-1/B-1", "FAIL" if venv_relative else "PASS",
           "gate still resolves _VENV_BIN = Path(sys.executable).parent" if venv_relative
           else "gate resolves a repository path")

    # Step 3 — gate version == image version?
    rc_g, gate_ver = run(MAIN, ".venv/bin/squawk", "--version")
    ...
    record(3, "V-3/B-1", "PASS" if same else "FAIL",
           f"gate={gate_ver.strip()} image={img_ver.strip()}")
    ...
    reqs = "\n".join(p.read_text() for p in MAIN.glob("requirements*.txt"))
    pinned = "squawk" in reqs.lower() or "sqlfluff" in reqs.lower()
    ...
    registered = "[" in (q.split("\n")[0] if q else "") and '"id"' in q
    ...
    notes = (f"pre_fix: SPR-1137 pre-fix state, TP-206 run 2026-09-18. "
